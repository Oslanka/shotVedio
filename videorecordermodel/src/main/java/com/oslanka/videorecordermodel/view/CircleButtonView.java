package com.oslanka.videorecordermodel.view;
...
import com.oslanka.videorecordermodel.R;
...
public class CircleButtonView extends View {
    ...
    private long mLongClickTime = 200;//长按最短时间(毫秒)，
    private int MAX_TIME = 10;//录制最大时间s
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mMinTime = a.getInt(R.styleable.CircleButtonView_minTime, 2);
        MAX_TIME = a.getInt(R.styleable.CircleButtonView_maxTime, 10);
        ...
        mProgressAni.setDuration(MAX_TIME * 1000);
    }
            ...
            switch (msg.what) {
                case WHAT_LONG_CLICK:
                    ...
                    if (mFinishListener != null) {
                        mFinishListener.onLongClick();
                        mVideoManage.prepareVideo();
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mEndTime - mStartTime < mLongClickTime) {
                    ...
                    if (mProgressAni != null && mProgressAni.getCurrentPlayTime() / 1000 < mMinTime && !isMaxTime) {
                        if (mFinishListener != null) {
                            mVideoManage.cancel();
                            mFinishListener.onNoMinRecord(mMinTime);
                        }
                        mProgressAni.cancel();
                    } else {
                        mVideoManage.release();
                        if (mFinishListener != null && !isMaxTime) {
                            //回掉录制完成接口
                            mFinishListener.onRecordingFinish(mVideoManage.getCurrentFileName());
                        }
                    }
                }
                break;
    ...
    private void startProgressAnimation() {
        ...
        mProgressAni.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                //录制动画结束时，即为录制全部完成  
                if (mFinishListener != null && isPressed) {
                    isPressed = false;
                    isMaxTime = true;
                    //回掉录制完成接口
                    mVideoManage.release();
    ...
    public interface OnRecordingFinishListener {
        ...
        void onRecordingFinish(String fileName);
    }

    public OnRecordingFinishListener mFinishListener;

    public void setRecordingFinishListener(OnRecordingFinishListener mFinishListener) {
